...
const SideBar = ({ sideBarPosts }: { sideBarPosts: IPost[] }) => {
  ...
  const sideBarPost = sideBarPosts.map((sideBarPost) => {
    const { title, description, link, image, _id } = sideBarPost
      ...
      >
        <div>
          ...
          <div className="text-center pt-4 drop-shadow-lg">
            <h2 className="text-sm md:text-2xl text-blue drop-shadow-xl">
              {title}
            </h2>
    ...
    let sideBarPostNoImageOrLink = (
      <div
        ...
        key={_id}
      >
        <h2 className="text-base md:text-2xl text-blue drop-shadow-xl">
          {title}
        </h2>
