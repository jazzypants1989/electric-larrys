...
import Post, { IPost } from "../models/SliderPost"
...
export default function Home({
  ...
  let publishedPosts = posts.filter((post: IPost) => post.isPublished)
  let featuredPosts = publishedPosts.filter((post: IPost) => post.isFeatured)
  let nonFeaturedPosts = publishedPosts
    .filter((post: IPost) => !post.isFeatured)
    .slice(0, 4)
  let featuredProducts = products
    .filter((product: IPost) => product.isFeatured)
    .slice(0, 4)
  let announcementList = announcements.map((announcement: IAnnouncement) => (
    ...
    <>
      <Layout title="Home">
        <div
          ...
          className="px-2 flex flex-col justify-center items-center gap-1 max-w-screen w-full bg-blue"
        >
          {announcementList}
        </div>
        <aside className="mx-auto overflow-hidden">
          ...
          <div className="mt-4 md:mt-0 flex gap-2 flex-col justify-center items-center overflow-hidden">
            {featuredProducts.length > 0 && (
              <div className="flex flex-col justify-center items-center">
                ...
                <div className="bg-orange bg-opacity-70 hover:bg-opacity-80 rounded-4xl md:rounded-l-full flex flex-col justify-center items-center transition-all duration-1000 object-contain aspect-auto w-auto md:w-3/4 lg:w-2/3">
